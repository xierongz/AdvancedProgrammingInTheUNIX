...
- 用标准I/O函数fgets从标准输入一次读取一行。当键入文件结束符(通常是Ctrl+D)作为行的第一个字符时，fgets返回一个NULL指针，进程终止。[第十八章](#第十八章)将说明所有特殊的终端字符(文件结束、退格字符、整行擦除等)，以及如何改变它们。
...
- 在子进程中，调用execlp以执行从标准输入读入的命令。在UNIX系统中，fork和exec两个独立的函数组合产生(spawn)一个新进程。[第八章](#第八章)详细说明。
...
&emsp;　控制线程的函数与控制进程的函数类似，在[第十二章](#第十二章)说明进程模型和线程模型的交互。
